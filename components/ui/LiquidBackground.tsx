...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      old = new THREE.Vector2(0, 0);
      diff = new THREE.Vector2(0, 0); // Own diff
      coords = new THREE.Vector2(0, 0); // Own coords
      ...
      constructor(
        ...
        this.pickNewTarget();
        this.coords = this.current; // Alias
      }
      pickNewTarget() {
        const r = Math.random;
      ...
      forceStop() {
        // No-op, we want it always running
      }
      update() {
        if (!this.enabled) return;
        const now = performance.now();
        
        const timeSinceInteract = now - this.manager.lastUserInteraction;
        let ramp = 0;
        if (timeSinceInteract < this.resumeDelay) {
            ramp = 0;
        } else if (timeSinceInteract < this.resumeDelay + this.rampDurationMs) {
             ramp = (timeSinceInteract - this.resumeDelay) / this.rampDurationMs;
        ...
        this.diff.subVectors(this.current, this.old);
      }
    }
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        if (!cursor) return; // Should not happen if called correctly

        const forceFactor = (props.mouse_force || 0) * (cursor.forceScale || 1.0);
        const forceX = (cursor.diff.x / 2) * forceFactor;
        const forceY = (cursor.diff.y / 2) * forceFactor;
        ...
        // Use cursor.coords instead of global Mouse.coords
        const centerX = Math.min(
          Math.max(cursor.coords.x, -1 + cursorSizeX + cellScale.x * 2),
          1 - cursorSizeX - cellScale.x * 2
        );
        const centerY = Math.min(
          Math.max(cursor.coords.y, -1 + cursorSizeY + cellScale.y * 2),
    ...
    class Simulation {
      ...
      update(cursors: any[] = []) {
        ...
        // Render external force for EACH cursor
    ...
    class Output {
      ...
      update(cursors: any[]) {
        this.simulation.update(cursors);
        this.render();
      }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      render() {
        ...
        if (Mouse.diff.x !== 0 || Mouse.diff.y !== 0) {
            cursors.push({ coords: Mouse.coords, diff: Mouse.diff, forceScale: 1.0 });
        }
        
        // Auto Driver
        if (this.autoDriver && this.autoDriver.enabled) {
             // Pass scale if needed, e.g. autoIntensity
             const intensity = this.autoDriver.mouse ? this.autoDriver.mouse.autoIntensity : 1.0;
             if (this.autoDriver.diff.x !== 0 || this.autoDriver.diff.y !== 0) {
                 cursors.push({ coords: this.autoDriver.coords, diff: this.autoDriver.diff, forceScale: intensity });
             } 
        }

        this.output.update(cursors);
      }
      loop() {
